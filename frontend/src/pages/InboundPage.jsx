    ...
    <>
      ...
      <section className="work-grid">
        <div className="panel form-panel">
          ...
          {mode === "single" && (
            <form onSubmit={submitSingle} style={{ opacity: submitting ? 0.6 : 1 }}>
              <h2>入库登记</h2>
              <label>运单号<input name="tracking_no" value={form.tracking_no} onChange={updateField} required disabled={submitting} /></label>
              <label>寄件方<input name="sender_name" value={form.sender_name} onChange={updateField} required disabled={submitting} /></label>
              <label>收件人<input name="receiver_name" value={form.receiver_name} onChange={updateField} required disabled={submitting} /></label>
              <label>手机号<input name="receiver_phone" value={form.receiver_phone} onChange={updateField} required disabled={submitting} /></label>
              <label>承运商<input name="carrier" value={form.carrier} onChange={updateField} required disabled={submitting} /></label>
              <label>
                柜格尺寸
                <select name="size" value={form.size} onChange={updateField} disabled={submitting}>
                  <option value="small">小</option>
                  <option value="medium">中</option>
                  <option value="large">大</option>
                </select>
              </label>
              <label>备注<input name="note" value={form.note} onChange={updateField} disabled={submitting} /></label>
              <button type="submit" disabled={submitting}>
                <PackagePlus size={18} />{submitting ? "提交中..." : "确认入库"}
              </button>
          ...
          {mode === "bulk" && (
            <form onSubmit={submitBulk}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
                ...
                <button type="button" className="ghost" onClick={addBulkItem} disabled={submitting}>
                  <Plus size={16} /> 添加一票
                </button>
              </div>
              ...
              <button type="submit" style={{ marginTop: "12px" }} disabled={submitting}>
                <PackagePlus size={18} />
                {submitting ? "提交中..." : `批量入库 (${bulkItems.length} 票)`}
              </button>
              <MessageBox type="success">{message}</MessageBox>
              <MessageBox type="error">{error}</MessageBox>
            </form>
